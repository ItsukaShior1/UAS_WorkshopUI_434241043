...
@section('page-title', 'Tulis Post')

@section('dashboard-content')
@if ($errors->any() || session('create_modal'))
    <div class="flash-error">
        <ul>@foreach ($errors->all() as $err)<li>{{ $err }}</li>@endforeach</ul>
    </div>
@endif

<div id="communityCreate" class="page-content active">
    <header class="create-header">
        <a href="{{ route('dashboard.community') }}" class="back-btn" aria-label="Kembali">
            <i data-lucide="arrow-left"></i>
        </a>
        <div>
            <h2>Bagikan Cerita Anda</h2>
            <p>Berbagi pengalaman Anda dapat membantu UMKM lain tumbuh bersama.</p>
        </div>
    </header>
    ...
    <form method="POST" action="{{ route('dashboard.community.store') }}" novalidate>
            ...
            <div class="category-grid">
                @foreach ($categories as $cat)
                    <label class="cat-option">
                        <input type="radio" name="category" value="{{ $cat }}" @checked(old('category') === $cat) required>
        ...
        <div class="form-section">
            ...
            <input type="text" name="title" value="{{ old('title') }}" required maxlength="160" class="text-input" placeholder="Contoh: Cara Saya Meningkatkan Penjualan...">
            @error('title')<small class="form-error">{{ $message }}</small>@enderror
        </div>

        <div class="form-section">
            <label class="form-label">Isi Cerita</label>
            <textarea name="content" required minlength="25" class="text-input textarea-large" placeholder="Tulis cerita, pengalaman, atau tips Anda di sini...">{{ old('content') }}</textarea>
        ...
        <button type="submit" class="submit-btn">Bagikan Cerita</button>
    </form>
</div>
@endsection
...
.submit-btn:hover { background: #6f9b80; }
</style>
@endpush

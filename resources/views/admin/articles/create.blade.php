...
@section('page-title', 'Buat Artikel Komunitas')
...
<div class="admin-form-page">
    <div class="admin-page-header">
        <div>
            ...
            <h2>Artikel Komunitas Baru</h2>
            <p>Tulis artikel untuk dipublikasikan di halaman Komunitas pengguna.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.articles.store') }}" enctype="multipart/form-data" class="admin-form-card">
        @csrf
        <div class="form-row">
        ...
        <div class="form-row">
            ...
            <input type="text" name="title" value="{{ old('title') }}" required maxlength="180" class="admin-input">
        </div>
        <div class="form-row">
            <label>Ringkasan (Excerpt)</label>
            <textarea name="excerpt" rows="3" maxlength="500" class="admin-input">{{ old('excerpt') }}</textarea>
            <small class="form-help">Maksimal 500 karakter. Jika kosong, akan dibuat otomatis dari isi.</small>
        </div>
        <div class="form-row">
            ...
            <small class="form-help">JPG/PNG/WebP, maksimal 2MB.</small>
        </div>
        <div class="form-row">
            <label>Isi Artikel <span class="req">*</span></label>
            <textarea name="content" rows="12" required minlength="25" class="admin-input admin-textarea-large">{{ old('content') }}</textarea>
        </div>
        <div class="form-row">
            <label class="switch">
                <input type="checkbox" name="is_published" value="1" @checked(old('is_published'))>
                <span class="switch-slider"></span>
                <span class="switch-label">Publikasikan sekarang</span>
            </label>
        </div>
